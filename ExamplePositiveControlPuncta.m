function [distance_pc_sort_M9V7Z1A1, distance_pc_avgMIN_M9V7Z1A1, distance_pc_avgMIN_hist_M9V7Z1A1] = ExamplePositiveControlPuncta(claudin5_full_M9V7Z1A1, coord_p_centers_M9V7Z1A1, PunctaIndex_M9V7Z1A1)
...
xycoord_PC_M9V7Z1A1 = horzcat(keeperX,keeperY);


N_puncta=20; %MANUALLY ADJUST THIS. Number of HIEL that are filled with punctate in the image 
xycoord_PC_M9V7Z1A1 = datasample(xycoord_PC_M9V7Z1A1, N_puncta);

Missing_values = numberOfPoints - length(xycoord_PC_M9V7Z1A1);

[mmm_pc nnn_pc] = size(xycoord_PC_M9V7Z1A1);
nnn_pc = mmm_pc;

[mmmm nnnn] = size(claudin5_full_M9V7Z1A1);
distance_pc_M9V7Z1A1 = zeros(nnn_pc,mmm_pc);

for i = 1 : nnn_pc % shorter, "coord_p_centers_M9V7Z1A1"
    for j = 1 : mmmm %longer, "claudin5"
        scale = 8.6719;
        distance_pc_M9V7Z1A1(i, j) = (sqrt(( xycoord_PC_M9V7Z1A1(i, 1) - claudin5_full_M9V7Z1A1(j,1)) ^ 2 + ...
            (xycoord_PC_M9V7Z1A1(i, 2) - claudin5_full_M9V7Z1A1(j,2)) ^ 2))/scale;
    end
end
...
% future analyses.
Total = 100;
distance_pc_sort_M9V7Z1A1 = distance_pc_sort_old(1:Total, 1:N_puncta); % take top 1000 values from this array 
...
hold on;
 ...
 hold on;
plot(xycoord_PC_M9V7Z1A1(:,1), xycoord_PC_M9V7Z1A1(:,2),'.','MarkerSize',12, 'Color', 'b'); hold on;
xlim([0 1024]);
ylim([0 1024]);
